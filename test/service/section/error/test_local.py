...
class TestServiceSectionErrorLocal:
    def test_simple(self):
        tl = LocalErrorInfoTemplate(local='./r.md')

        assert tl.local == './r.md'

    def test_simple_repr(self):
        tl = LocalErrorInfoTemplate(local='./r.md')

        assert repr(tl) == "<LocalErrorInfoTemplate local: './r.md'>"

    def test_call(self):
        tl = LocalErrorInfoTemplate(local='./${DIR}/r.md')

        with tempfile.TemporaryDirectory() as wtd:
            t = tl(workdir=wtd, environ=dict(DIR='1/2/3'))

            assert t.local == os.path.join(wtd, '1/2/3', 'r.md')
